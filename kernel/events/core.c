...
#ifdef CONFIG_FREQ_PROMOTION
SYSCALL_DEFINE2(samm_start, pid_t, pid, int, node){
	kfreqmond_init(pid, node);
	return 0;
}
SYSCALL_DEFINE1(samm_end, pid_t, pid){
	kfreqmond_exit();
	return 0;
}
SYSCALL_DEFINE1(samm_update, int, period){
	pebs_update_period(period);
	return 0;
}
int samm_perf_event_init(struct perf_event *event, unsigned long nr_pages){
	struct perf_buffer *rb = NULL;
    int ret = 0, flags = 0;
 ...
 * @flags:		perf event open flags
 */
int samm_perf_event_open(struct perf_event_attr *attr_ptr, pid_t pid,
	...
	// if (err)
	// 	return err;
	attr = *attr_ptr; // kernel space to kernel space.
